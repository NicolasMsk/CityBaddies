# Deploiement Cloud Run Job - Generation Guides d'Achat (via Cloud Build)
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Generation Guides d'Achat..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "generate-buying-guide"
...
gcloud builds submit --config=cloudbuild-buying-guide.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  Creation du scheduler (10h00 tous les jours)..." -ForegroundColor Gray
...
Write-Host ""
Write-Host "Generation Guides d'Achat deploye!" -ForegroundColor Green
Write-Host ""
