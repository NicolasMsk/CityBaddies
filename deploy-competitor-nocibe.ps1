# Deploiement Cloud Run Job - Competitor Nocibe (via Cloud Build)
# Cherche les prix Nocibe pour tous les deals LIVE non-Nocibe
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Competitor Nocibe..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "competitor-nocibe"
...
gcloud builds submit --config=cloudbuild-competitor-nocibe.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
Write-Host ""
Write-Host "Competitor Nocibe deploye!" -ForegroundColor Green
Write-Host ""
