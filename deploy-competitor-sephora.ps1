# Deploiement Cloud Run Job - Competitor Sephora (via Cloud Build)
# Cherche les prix Sephora pour tous les deals LIVE non-Sephora
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Competitor Sephora..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "competitor-sephora"
...
gcloud builds submit --config=cloudbuild-competitor-sephora.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
Write-Host ""
Write-Host "Competitor Sephora deploye!" -ForegroundColor Green
Write-Host ""
