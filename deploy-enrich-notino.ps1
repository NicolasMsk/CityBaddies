# Deploiement Cloud Run Job - Enrichissement Notino (via Cloud Build)
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Enrichissement Notino..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "enrich-notino"
...
gcloud builds submit --config=cloudbuild-enrich-notino.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  Creation du scheduler (8h45 tous les jours)..." -ForegroundColor Gray
...
Write-Host ""
Write-Host "Enrichissement Notino deploye!" -ForegroundColor Green
Write-Host ""
