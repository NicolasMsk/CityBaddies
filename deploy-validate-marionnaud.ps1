# Deploiement Cloud Run Job - Validation Marionnaud (via Cloud Build)
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Validation Marionnaud..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "validate-marionnaud"
...
gcloud builds submit --config=cloudbuild-validate-marionnaud.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
Write-Host ""
Write-Host "Validation Marionnaud deploye!" -ForegroundColor Green
Write-Host ""
