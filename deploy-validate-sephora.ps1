# Deploiement Cloud Run Job - Validation Sephora (via Cloud Build)
$ErrorActionPreference = "Stop"

Write-Host "Deploiement Validation Sephora..." -ForegroundColor Cyan

$REGION = "europe-west1"
$PROJECT = "city-baddies"
$REGISTRY = "$REGION-docker.pkg.dev/$PROJECT/citybaddies-scrapers"
$JOB_NAME = "validate-sephora"
...
gcloud builds submit --config=cloudbuild-validate-sephora.yaml --substitutions=_IMAGE_TAG="$REGISTRY/${JOB_NAME}:latest"
...
# Creer/mettre a jour le scheduler pour 9h tous les jours
...
Write-Host ""
Write-Host "Validation Sephora deploye!" -ForegroundColor Green
Write-Host ""
