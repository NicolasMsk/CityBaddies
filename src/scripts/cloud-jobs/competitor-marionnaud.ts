/**
 * =============================================================================
 * COMPETITOR-MARIONNAUD.TS — Cloud Run Job
 * =============================================================================
 *
 * Lancé tous les jours à 9h.
 * Récupère TOUS les deals LIVE qui ne viennent PAS de Marionnaud,
 * cherche le même produit sur Marionnaud via marionnaud-search.ts,
...
import { searchMarionnaudPrice, extractMarionnaudPriceFromUrl } from '../../lib/scraping/marionnaud-search';
...
async function main() {
  ...
  console.log('🚀 [CLOUD JOB] Competitor Marionnaud — Prix concurrents sur marionnaud.fr');
  ...
  try {
    ...
    if (!marionnaud) {
      console.error('❌ Merchant Marionnaud introuvable');
      process.exit(1);
    }
    ...
    const deals = await prisma.deal.findMany({
      ...
      include: {
        ...
        competitorPrices: {
          where: { merchantId: marionnaud.id },
        },
      },
      orderBy: { score: 'desc' },
      take: MAX_DEALS,
    ...
    console.log(`\n📦 ${deals.length} deals LIVE (hors Marionnaud) à traiter\n`);
    ...
    for (let i = 0; i < deals.length; i++) {
      ...
      try {
        ...
        if (existing?.productUrl) {
          ...
          result = await extractMarionnaudPriceFromUrl(existing.productUrl, volume || undefined);
        } else {
          const searchQuery = cleanSearchQuery(`${brand} ${productName}`, brand);
          console.log(`  🔍 Query: "${searchQuery}"`);
          result = await searchMarionnaudPrice(searchQuery, volume || undefined, {
            brand,
            name: productName,
            category,
          });
        ...
        if (result.found && result.currentPrice) {
          ...
          if (diff < 0) {
            console.log(`  ✅ ${result.currentPrice}€ sur Marionnaud — 🏆 ${Math.abs(diff).toFixed(2)}€ moins cher ici (${Math.abs(diffPct)}%)`);
          } else if (diff > 0) {
            console.log(`  ✅ ${result.currentPrice}€ sur Marionnaud — ⚠️ ${diff.toFixed(2)}€ plus cher ici (+${diffPct}%)`);
          } else {
            console.log(`  ✅ ${result.currentPrice}€ sur Marionnaud — ➡️ Prix identique`);
          }
          console.log(`  💾 Sauvegardé en BDD`);

          stats.found++;
        } else {
          console.log(`  ❌ Non trouvé sur Marionnaud: ${result.error || '?'}`);
          stats.notFound++;
        }
      } catch (err) {
    ...
    console.log(`\n${'═'.repeat(70)}`);
    console.log(`  📊 RAPPORT — COMPETITOR MARIONNAUD`);
    ...
    console.log(`${'═'.repeat(70)}`);
    console.log(`\n✅ [CLOUD JOB] Competitor Marionnaud terminé!`);
