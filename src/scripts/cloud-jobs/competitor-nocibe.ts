/**
 * =============================================================================
 * COMPETITOR-NOCIBE.TS — Cloud Run Job
 * =============================================================================
 *
 * Lancé tous les jours à 9h.
 * Récupère TOUS les deals LIVE qui ne viennent PAS de Nocibé,
 * cherche le même produit sur Nocibé via nocibe-search.ts,
...
import { searchNocibePrice, extractNocibePriceFromUrl } from '../../lib/scraping/nocibe-search';
...
async function main() {
  ...
  console.log('🚀 [CLOUD JOB] Competitor Nocibé — Prix concurrents sur nocibe.fr');
  ...
  try {
    // Récupérer le merchant Nocibé
    const nocibe = await prisma.merchant.findFirst({ where: { slug: 'nocibe' } });
    if (!nocibe) {
      console.error('❌ Merchant Nocibé introuvable');
      process.exit(1);
    }

    // Tous les deals ACTIVE qui ne sont PAS de Nocibé
    const deals = await prisma.deal.findMany({
      where: {
        status: 'ACTIVE',
        product: {
          merchant: { slug: { not: 'nocibe' } },
        },
      },
      include: {
        product: {
    ...
    console.log(`\n📦 ${deals.length} deals LIVE (hors Nocibé) à traiter\n`);
    ...
    for (let i = 0; i < deals.length; i++) {
      ...
      try {
        ...
        if (existing?.productUrl) {
          ...
          result = await extractNocibePriceFromUrl(existing.productUrl, volume || undefined);
        } else {
          // Sinon, flow complet (search + GPT + extraction)
          const searchQuery = cleanSearchQuery(`${brand} ${productName}`, brand);
          console.log(`  🔍 Query: "${searchQuery}"`);
          result = await searchNocibePrice(searchQuery, volume || undefined, {
            brand,
            name: productName,
            category,
          });
        ...
        if (result.found && result.currentPrice) {
          ...
          if (diff < 0) {
            console.log(`  ✅ ${result.currentPrice}€ sur Nocibé — 🏆 ${Math.abs(diff).toFixed(2)}€ moins cher ici (${Math.abs(diffPct)}%)`);
          } else if (diff > 0) {
            console.log(`  ✅ ${result.currentPrice}€ sur Nocibé — ⚠️ ${diff.toFixed(2)}€ plus cher ici (+${diffPct}%)`);
          } else {
            console.log(`  ✅ ${result.currentPrice}€ sur Nocibé — ➡️ Prix identique`);
          }
          console.log(`  💾 Sauvegardé en BDD`);

          stats.found++;
        } else {
          console.log(`  ❌ Non trouvé sur Nocibé: ${result.error || '?'}`);
          stats.notFound++;
        }
      } catch (err) {
    ...
    console.log(`\n${'═'.repeat(70)}`);
    console.log(`  📊 RAPPORT — COMPETITOR NOCIBÉ`);
    ...
    console.log(`${'═'.repeat(70)}`);
    console.log(`\n✅ [CLOUD JOB] Competitor Nocibé terminé!`);
