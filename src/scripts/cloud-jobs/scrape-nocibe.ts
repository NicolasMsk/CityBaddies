/**
 * Cloud Run Job - Scrape Nocibé
 * Exécuté quotidiennement pour alimenter la base de données
 * 
 * Ce script réutilise la logique de import-nocibe.ts
 */
...
async function main() {
  ...
  console.log('🚀 [CLOUD JOB] Scraping Nocibé...');
  ...
  try {
    ...
    if (scrapingSources.length === 0) {
      console.log('⚠️ Aucune source de scraping active pour Nocibé');
      return;
    }
    ...
    // Filtrer les produits avec volume
    const productsWithSize = allProducts.filter(p => p.size);
    console.log(`\n📊 ${allProducts.length} produits scrapés, ${productsWithSize.length} avec volume`);
        ...
        try {
          ...
          await prisma.deal.create({
            data: {
              ...
              refinedTitle: `${product.brand} ${product.name} (${product.size})`.substring(0, 200),
              dealPrice: product.currentPrice,
              originalPrice: product.originalPrice || product.currentPrice,
              discountPercent: product.discountPercent || 0,
              discountAmount: (product.originalPrice || product.currentPrice) - product.currentPrice,
