 ...
 * Pour chaque deal, il:
 * 1. Ouvre la page produit sur marionnaud.fr (Playwright)
 * 2. Scrape le prix actuel, le prix barré, la promo
...
// Fonction locale pour calculer le prix par unité
...
// Type pour les données scrapées d'une page produit Marionnaud
...
class MarionnaudProductScraper {
  ...
  /**
   * Ferme les popups de cookies OneTrust / Didomi
   */
  ...
  /**
   * Scrape une page produit Marionnaud directement
   */
  async scrapeProduct(productUrl: string): Promise<ScrapedProductInfo | null> {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        // Prix — prix actuel (promo ou normal)
        ...
        for (const sel of strikeSelectors) {
          ...
          if (el) {
            originalPriceText = (el.textContent || '').replace(/\u00a0/g, ' ');
            break;
          }
        }
        ...
        const hasPromo = !!(originalPriceText || promoBadge);
...
async function main() {
  ...
  // Récupérer les deals ACTIVE + PENDING de Marionnaud
  let deals;
  if (specificDealId) {
  ...
  // Initialiser le scraper Playwright
  ...
  try {
    for (let i = 0; i < deals.length; i++) {
      const deal = deals[i];
      console.log(`\n${'═'.repeat(60)}`);
      console.log(`[${i + 1}/${deals.length}] Deal #${deal.id}: ${deal.title?.substring(0, 60)}...`);
      ...
      // URL du produit sur marionnaud.fr
      ...
      // Aller sur la page produit et scraper le prix
      ...
      // Délai entre les requêtes (1.5-3s aléatoire)
      if (i < deals.length - 1) {
