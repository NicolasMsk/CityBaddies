/**
 * Script local d'enrichissement des deals Notino
 * 
 * Usage:
 *   npx tsx src/scripts/enrich-notino.ts              # Traite tous les deals Notino actifs
 *   npx tsx src/scripts/enrich-notino.ts --test       # Test sur 1 deal de la DB
 *   npx tsx src/scripts/enrich-notino.ts --limit 5    # Traite max 5 deals
 * 
...
async function scrapeNotinoPage(browser: Browser, url: string): Promise<ScrapedData | null> {
  ...
  try {
    ...
    if (variants.length === 0) {
      const singleVolume = await page.evaluate(() => {
        ...
        for (const span of spans) {
          const text = span.textContent?.trim() || '';
          if (/^\d+(?:[,.]\d+)?\s*(ml|g|l|cl|oz|kg)$/i.test(text)) return text;
        }
        return null;
      });
    ...
    await page.evaluate(() => {
      ...
      if (tabAnchor) tabAnchor.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
    ...
    return { brand: titleData.brand, name: titleData.name, fullTitle, description, ingredients, properties, variants };
  } catch (error) {
...
async function rewriteWithGPT(data: ScrapedData, priceInfo: PriceInfo): Promise<RewrittenContent | null> {
  try {
    const sizesInfo = data.variants.length > 0 ? data.variants.join(', ') : 'Taille unique ou non spécifié';
...
async function main() {
  ...
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('Enrichissement des deals Notino');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  console.log(`Deals Notino à enrichir: ${deals.length}\n`);

  if (deals.length === 0) {
    console.log('Aucun deal à enrichir.');
    await prisma.$disconnect();
    return;
  }

  // Lancer le navigateur (nouveau contexte par produit)
  ...
  try {
    for (let i = 0; i < deals.length; i++) {
      ...
      console.log(`[${i + 1}/${deals.length}] ${deal.title.substring(0, 60)}...`);
      console.log(`  URL: ${url}`);

      const scrapedData = await scrapeNotinoPage(browser, url);
      ...
      console.log(`    - Titre: ${scrapedData.fullTitle}`);
      console.log(`    - Variantes: ${scrapedData.variants.join(', ') || 'N/A'}`);
      console.log(`    - Description: ${scrapedData.description?.substring(0, 50) || 'Non'}...`);
      console.log(`    - Ingrédients: ${scrapedData.ingredients ? 'Oui' : 'Non'}`);
      console.log(`    - Propriétés: ${Object.keys(scrapedData.properties).join(', ') || 'Aucune'}`);
      ...
      if (rewritten) {
        ...
        console.log('  ⚠️ GPT échoué, utilisation des données brutes');
      }
