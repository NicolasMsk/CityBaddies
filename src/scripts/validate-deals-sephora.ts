/**
 * Script de validation des deals Sephora
 * 
 ...
 * 
 * Usage: npx tsx src/scripts/validate-deals-sephora.ts [--limit N] [--deal-id ID] [--headless]
 */

import { chromium, Browser, Page } from 'playwright';
import { PrismaClient } from '@prisma/client';
import { calculatePricePerUnit } from '../lib/utils/volume';
...
class SephoraProductScraper {
  ...
  async scrapeProductVariants(productUrl: string): Promise<ScrapedProductInfo | null> {
    ...
    try {
      // Délai aléatoire avant navigation (1-2s)
...
async function main() {
  ...
  let limit = 10;
  let specificDealId: number | null = null;
  let headless = false;
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (args[i] === '--deal-id' && args[i + 1]) {
      specificDealId = parseInt(args[i + 1]);
    }
    if (args[i] === '--headless') {
      headless = true;
    }
  ...
  console.log('🔍 Validation des deals Sephora');
  console.log('================================');
